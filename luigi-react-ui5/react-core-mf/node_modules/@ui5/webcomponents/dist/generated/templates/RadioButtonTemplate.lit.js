...
const block0 = (context, tags, suffix) => html`<div class="ui5-radio-root" role="radio" aria-checked="${ifDefined(context.checked)}" aria-disabled="${ifDefined(context.effectiveAriaDisabled)}" aria-describedby="${ifDefined(context.effectiveAriaDescribedBy)}" aria-label="${ifDefined(context.ariaLabelText)}" tabindex="${ifDefined(context.effectiveTabIndex)}" @click="${context._onclick}" @keydown="${context._onkeydown}" @keyup="${context._onkeyup}" @mousedown="${context._onmousedown}" @mouseup="${context._onmouseup}" @focusout="${context._onfocusout}"><div class='ui5-radio-inner ${classMap(context.classes.inner)}'><svg class="ui5-radio-svg" focusable="false" aria-hidden="true">${blockSVG1(context, tags, suffix)}</svg><input type='radio' ?required="${context.required}" ?checked="${context.checked}" ?readonly="${context.readonly}" ?disabled="${context.effectiveAriaDisabled}" name="${ifDefined(context.name)}"  data-sap-no-tab-ref/></div>${ context.text ? block1(context, tags, suffix) : undefined }${ context.hasValueState ? block2(context, tags, suffix) : undefined }<slot name="formSupport"></slot></div>`;
